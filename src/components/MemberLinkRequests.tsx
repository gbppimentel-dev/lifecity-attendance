...
export default function MemberLinkRequests({owner=false,linked=false,onChanged}:{owner?:boolean;linked?:boolean;onChanged:()=>void}) {
 ...
 return <section className={'lcr-panel'+(owner?' lcr-owner':'')} ref={heading} data-change-id="LC-UI-COPY-v2">
  ...
  {owner && <div className="lcr-toolbar"><label>Requests<ResponsiveSelect value={status} disabled={saving} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="pending">Pending Review</option><option value="approved">Approved</option><option value="declined">Declined</option><option value="cancelled">Cancelled</option><option value="all">All Requests</option></ResponsiveSelect></label><span aria-live="polite">{loading||searching ? 'Updating…' : `${pendingCount} Pending · ${total} Matching`}</span></div>}
...
function RequestReview({request:r,onCancel,onDone,onBusy}:{request:RequestRow;onCancel:()=>void;onDone:()=>void;onBusy:(v:boolean)=>void}) {
 ...
 return <div className="lcr-review">
  ...
  {error && <p className="lcr-error" role="alert">{uiMessage(error)}</p>}
  {decision ? <div className="lcr-confirm"><strong>{decision==='approved'?'Approve and Connect This Member?':'Decline This Request?'}</strong><p>{decision==='approved'?'This connects the two identities shown above. Role, contact details and attendance stay unchanged.':'The applicant will see your message and can submit corrected details.'}</p><div className="lcr-actions lcr-decision-actions"><button className="lcr-button" disabled={saving} onClick={()=>setDecision(null)}>Back</button><button className={"lcr-button "+(decision==='approved'?'lcr-primary':'lcr-decline')} disabled={saving||(decision==='declined'&&note.trim().length<3)} onClick={()=>void decide()}>{saving?'Saving…':'Confirm '+(decision==='approved'?'Approval':'Decline')}</button></div></div> :
  <div className="lcr-actions lcr-review-actions"><button className="lcr-button" disabled={saving} onClick={onCancel}><X size={14}/>Close Review</button><button className="lcr-button lcr-decline" disabled={saving||note.trim().length<3} onClick={()=>setDecision('declined')}><X size={15}/>Decline</button><button className="lcr-button lcr-primary" disabled={saving||!candidate||!verified} onClick={()=>setDecision('approved')}><Check size={15}/>Approve Link</button></div>}
 </div>
}
