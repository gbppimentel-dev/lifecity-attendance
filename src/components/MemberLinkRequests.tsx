...
export default function MemberLinkRequests({owner=false,linked=false,onChanged}:{owner?:boolean;linked?:boolean;onChanged:()=>void}) {
 ...
 return <section className={'lcr-panel'+(owner?' lcr-owner':'')} ref={heading} data-change-id="LC-UI-COPY-v2">
  ...
  {loading ? <p className="lcr-empty" role="status">Loading Requests…</p> : !error && (owner ? <>
   ...
   <footer className="lcr-pagination"><span>Page {actualPage} of {pages} · 25 Per Page</span><div><button className="lcr-button" disabled={saving||actualPage===1} onClick={()=>turn(1)}>First</button><button className="lcr-button" disabled={saving||actualPage===1} onClick={()=>turn(actualPage-1)}>Previous</button><button className="lcr-button" disabled={saving||actualPage>=pages} onClick={()=>turn(actualPage+1)}>Next</button><button className="lcr-button" disabled={saving||actualPage>=pages} onClick={()=>turn(pages)}>Last</button></div></footer>
  </> : <>
