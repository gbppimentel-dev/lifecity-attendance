...
export default function FeedbackInbox(){
 ...
 <div className="lcf-filters"><label>Status<select value={status} disabled={!!busy} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="all">All Statuses</option>{statuses.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label><label>Category<select value={category} disabled={!!busy} onChange={e=>{setCategory(e.target.value);setPage(1)}}><option value="all">All Categories</option>{feedbackCategories.map(c=><option key={c.value} value={c.value}>{c.label}</option>)}</select></label><label className="lcf-search-label">Search<div className="lcf-search"><Search size={17}/><input type="search" maxLength={200} value={search} disabled={!!busy} onChange={e=>setSearch(e.target.value)} placeholder="Subject or message…"/></div></label></div>
 ...
 {error?<p className="lcf-error" role="alert">{error}</p>:loading||filtering?<p className="lcf-empty" role="status">Loading Feedback…</p>:!data?.rows.length?<div className="lcf-empty"><MessageSquare size={28}/><p>No Feedback Matches This View.</p></div>:<div className="lcf-feedback-list">{data.rows.map(row=>{
  ...
  <div className="lcf-ticket-controls">{row.status!=='resolved'&&<button type="button" className="lcf-ticket-action" disabled={blocked} onClick={()=>void changeStatus(row,'resolved')}><CheckCircle2 size={16}/>Mark Complete</button>}<label className="lcf-status">Status<select value={row.status} disabled={blocked} onChange={e=>void changeStatus(row,e.target.value)}>{statuses.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label></div>
  </article>
 })}</div>}
