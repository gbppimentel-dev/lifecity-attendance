...
export default function FeedbackWidget(){
 ...
 <header className="lcf-heading lcf-playful-heading"><div className="lcf-header-art" aria-hidden="true"><MessageSquare size={30}/><Sparkles size={16}/><i/><i/></div><div><p className="eyebrow">LifeCity Attendance · App Feedback</p><h2 id={`${id}-title`}>A Little Idea. A Better App.</h2><p>Send app feedback directly to the owner.</p></div><button type="button" className="lcf-icon" aria-label="Close Feedback" disabled={busy} onClick={close}><X size={20}/></button></header>
 ...
 <label>Subject<input required minLength={3} maxLength={160} value={subject} onChange={e=>setSubject(e.target.value)} placeholder="A short title…"/></label>
 <label>Message<textarea required minLength={10} maxLength={5000} rows={3} value={message} onChange={e=>setMessage(e.target.value)} placeholder={category==='bug'?'What happened, and on which page?':category==='idea'?'What’s your idea, and how would it help?':'What could we improve?'}/><small>{message.length.toLocaleString()} / 5,000</small></label>
 <label className="lcf-anonymous"><input type="checkbox" checked={anonymous} onChange={e=>setAnonymous(e.target.checked)} aria-describedby={`${id}-privacy`}/><span><strong>Hide My Identity</strong><small id={`${id}-privacy`}>Your name stays hidden unless the app owner reveals it.</small></span></label>
 </fieldset>
