...
import { Users, ArrowLeft, ArrowRight, CalendarDays, MapPin, RefreshCw, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight, QrCode, Heart } from 'lucide-react'
...
export default function GuestPreview({onBack,onSignIn,signingIn,error:authError}:{onBack:()=>void;onSignIn:()=>void;signingIn:boolean;error:string}) {
 ...
 return <main className="lc-guest" data-change-id="LC-P04B-v2-GOOGLE-LOGO">
  <header className="lcg-topbar"><div className="brand"><div className="brand-icon small"><Users size={20}/></div><span>LifeCity Attendance</span></div><button className="lcg-button" onClick={onBack} disabled={signingIn}><ArrowLeft size={16}/>Back to Welcome</button></header>
  <section className="lcg-hero"><div><p className="eyebrow">A Place to Belong</p><h1>Come as You Are.</h1><p>Find a gathering, meet your community, and make room for something meaningful.</p><span className="lcg-guest-pill">Guest Preview</span></div><div className="lcg-hero-note"><CalendarDays size={28}/><strong>See You at Church</strong><span>Explore what’s coming up.</span></div></section>
  <section className="lcg-services" ref={listRef} aria-labelledby="lcg-services-title" aria-busy={loading}>
   <header className="lcg-section-head"><div><p className="eyebrow">Gather with Us</p><h2 id="lcg-services-title">Current &amp; Upcoming Services</h2><p>Public gatherings · All times in Manila</p></div><div className="lcg-tools"><span aria-live="polite">{loading?'Updating…':`${data?.total??0} Services`}</span><button className="lcg-button" disabled={loading} onClick={()=>setRevision(v=>v+1)}><RefreshCw size={16}/>Refresh</button></div></header>
   {loading?<div className="lcg-grid" role="status"><span className="lcg-sr-only">Loading services…</span>{[0,1,2].map(n=><div className="lcg-skeleton" key={n} aria-hidden="true"/>)}</div>:error?<div className="lcg-empty" role="alert"><p>{error}</p><button className="lcg-button" onClick={()=>setRevision(v=>v+1)}>Try Again</button></div>:!data?.rows.length?<div className="lcg-empty"><CalendarDays size={30}/><h3>More Gatherings Soon</h3><p>There are no public services listed right now. Check back soon or ask the church team about the next gathering.</p></div>:<div className="lcg-grid" data-count={data.rows.length}>{data.rows.map(service=><article className="lcg-card" key={service.id}><div className="lcg-pills"><span className={'lcg-pill '+service.state}>{service.state==='in-progress'?'In Progress':'Upcoming'}</span>{service.is_sunday_service&&<span className="lcg-pill sunday">Sunday Service</span>}</div><h3>{service.name}</h3><div className="lcg-service-meta"><p><CalendarDays size={17}/><span>{dateLabel(service.starts_at)}</span></p><p><MapPin size={17}/><span>{service.location||'Ask the church team for the venue.'}</span></p></div></article>)}</div>}
